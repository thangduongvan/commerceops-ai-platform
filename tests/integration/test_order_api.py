...
# V5: the payment outcome is steered by the `payment` fixture in conftest.py
# (which patches gateway_client.charge), since V5 replaced the in-process
# random-coin-flip stub with a real HTTP call to the payment gateway.
...
# it here keeps every other test in this file focused on order/payment/stock
# logic without needing a real/mocked SQS; test_create_order_publishes_order_events
# below is the one test that actually asserts on it.
...
def test_create_order_success_path_decrements_stock_and_pays(client, payment):
...
def test_create_order_payment_failure_restocks_items(client, payment):
    customer_id = _create_customer(client)
    product_id = _create_product(client, stock=10)
    payment.declines()
...
def test_cancel_order_restocks_and_blocks_double_cancel(client, payment):
    customer_id = _create_customer(client)
    product_id = _create_product(client, stock=10)
    payment.approves()
...
def test_create_order_publishes_order_events(client, payment, published_events):
    customer_id = _create_customer(client)
    product_id = _create_product(client, stock=10)
    payment.approves()
...
def test_create_order_payment_failure_publishes_payment_failed_event(client, payment, published_events):
    customer_id = _create_customer(client)
    product_id = _create_product(client, stock=10)
    payment.declines()
...
def test_cancel_order_publishes_cancelled_event(client, payment, published_events):
    customer_id = _create_customer(client)
    product_id = _create_product(client, stock=10)
    payment.approves()
...
def test_list_orders_filters_by_customer(client, payment):
